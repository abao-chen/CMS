(function () {
    ...
    TableUtils = function (options) {
        ...
        _settings = {
            ...
            "ajax": function (data, callback) {
                var param = _settings.searchParas ? _settings.searchParas : _this.getSearchParams(data);
                param["method"] = _settings.searchMethod;
                //ajax请求数据
                $.ajax({
                    type: "POST",
                    url: _settings.url,
    ...
    TableUtils.prototype = {
        ...
        initTable: function () {
            ...
            this._tableObj = $('#' + _settings.tId).DataTable(_settings);
            this.initToolbar(_settings);
            this.initCheckAll();
            this.initShowSearchEven();
            return this;
        },
        ...
        getHeight: function () {
            console.log(window.innerHeight - 232)
        ...
        initToolbar: function (options) {
            ...
            if (!!options.isDelete) {
                $("div.toolbar").append('<a class="btn btn-default btn-sm" href="javascript:void(0);" onclick="curTable.delSelecteRows();"><span class="glyphicon glyphicon-trash" title="删除"></span></a>');
            }
            if (!!options.isExport) {
        ...
        reload: function () {
            this._tableObj.ajax.reload(null, false);
        },
        delSingleRow: function (ids) {
            bootAlert.confirm({
                message: "确认要删除选中数据吗？"
            }).on(function (result) {
                if (result) {
                    $.ajax({
                        ...
                        url: _settings.url,
                        cache: false, //禁用缓存
                        data: { "Id": ids, "method": _settings.delMethod }, //传入组装的参数
                        dataType: "json",
                        success: function (result) {
                            ajaxSuccessDone(result, function () {
                                toastr.success("删除成功！");
        ...
        download: function () {
            var data = this._tableObj.ajax.params();
            var param = this.getSearchParams(data);
            param["method"] = _settings.downloadMethod;
            $.ajax({
                type: "POST",
                url: _settings.url,
                cache: false, //禁用缓存
        ...
        delSelecteRows: function () {
            var ids = _this.getSelectedRowIds();
            if (ids !== "") {
                _this.delSingleRow(ids);
            } else {
                toastr.warning("请选择你要删除的数据！");
            }
        },
        add: function () {
            window.location = _settings.editUrl;
        },
        edit: function (id) {
            window.location = _settings.editUrl + "?Id=" + id;
        ...
        getSearchParams: function (data) {
            ...
            param.searchColunms = _settings.searchColunms;
            var $formObj = $("#searchPanel .form-control");
            $formObj.each(function (index, inputObj) {
                if ($(this).attr("SearchAttr") && $(this).val() !== "") {
                    switch ($(this)[0].tagName.toUpperCase()) {//表单元素类型
