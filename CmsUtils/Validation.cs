...
namespace CmsUtils
{
	...
	public class Validation
	{
		...
		public static bool IsDigit(string chkString) 
		{
			...
			{
				char[] checkChar =  chkString.ToCharArray();
				foreach(char ch in  checkChar)
				{
					if(!Char.IsDigit(ch))
					{
						result = false;
		...
		private static void ValidMapping(Page page, object sender)
		{
			...
			IEnumerator validators = page.Validators.GetEnumerator();
			while (validators.MoveNext())
			{
				BaseValidator validator = (BaseValidator)validators.Current;
		...
		public static bool StringToBytes(string chkString, int maxLength, int minLength) 
		{
			...
			int iLength = sjisEnc.GetByteCount(chkString);
		...
		public static bool IsDateTimeString( string strParam )
		{
			try
			{
				DateTime date = DateTime.Parse( strParam );
				return true;
			}
