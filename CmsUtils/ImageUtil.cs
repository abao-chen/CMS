...
namespace CmsUtils
{
    public class ImageUtil
    {
        ...
        public static void CreatePic(string FromImagePath, string ToImagePath, double MaxWidth, double MaxHeight)
        {
            ...
            var Width = 0; //框条和卡纸的宽度
            var Height = 0; //框条和卡纸的高度
            double wmp = 1, hmp = 1, default_pparm = 1; //默认宽的比例，默认高对应的比例，最大宽度比例，最大高度比例，默认实际比例，最大实际比例
            Image imgPic = null; //画芯
            imgPic = Image.FromFile(FromImagePath);
            Width = imgPic.Width;
            ...
            tmp = new Bitmap(Width, Height); //最大的容器
            g = Graphics.FromImage(tmp);
            //g.FillRectangle(new SolidBrush(Color.White), new Rectangle(0, 0, tmp.Width, tmp.Height));
            var Boxs = new Rectangle(0, 0, imgPic.Width, imgPic.Height);
            var Boxd = new Rectangle(0, 0,
                Width, Height); //算图的起点
